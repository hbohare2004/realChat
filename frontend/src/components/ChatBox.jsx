...
const ChatBox = ({ selectedRoom, onBack }) => {
  ...
  // Join room when selectedRoom changes
  useEffect(() => {
    if (selectedRoom) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      setLoading(true);
      try {
        const response = await api.get(`/messages/${selectedRoom._id}`);
  ...
  if (selectedRoom.type === 'direct') {
    displayUser = selectedRoom.participants.find(p => p._id !== user._id);
  }
  const displayName = displayUser ? displayUser.username : selectedRoom.name;
  const displayAvatar = displayUser?.avatar;

  const isTyping = displayUser ? typingUsers.includes(displayUser._id) : false;
    ...
    <div className="flex flex-col w-full h-full glass-panel m-0 md:m-4 md:ml-4 rounded-none md:rounded-2xl overflow-hidden relative z-10">
      ...
      <div className="px-4 py-3 border-b border-gray-100 dark:border-dark-700 bg-white/50 dark:bg-dark-800/50 backdrop-blur-sm flex items-center justify-between shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-gray-800 dark:text-gray-100 font-bold text-lg leading-tight">{displayName}</h3>
      ...
      {/* Messages */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-white/30 dark:bg-dark-900/30">
        {loading ? (
              ...
              messages.map((msg, index) => {
                ...
                return (
                  <div key={msg._id || index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                    ...
                    >
                      <p className="leading-relaxed whitespace-pre-wrap">{msgContent}</p>
      ...
      <div className="p-3 md:p-4 bg-white/50 dark:bg-dark-800/50 backdrop-blur-sm border-t border-gray-100 dark:border-dark-700 shrink-0">
        <MessageInput selectedRoom={selectedRoom} displayUser={displayUser} />
      </div>
    </div>
  );
};
